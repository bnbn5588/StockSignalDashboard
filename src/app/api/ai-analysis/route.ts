...
export async function GET() {
  ...
  const cacheKey = `ai-analysis:${today}`;

  try {
    // Check Redis cache — shared across all serverless instances.
    // If today's analysis exists, return it immediately (no Claude call).
    const cached = redis ? await redis.get(cacheKey) : null;
    if (cached) return Response.json(JSON.parse(cached));
  } catch {
    // Redis unreachable — skip cache, call Claude.
  }
...
- Do not invent data not present above.`;
    ...
    const result = { ...analysis, model: "claude-opus-4-8", fetchedAt: new Date().toISOString(), prompt };

    // Store in Redis — expires after 25 hours so it never bleeds into the next day.
    try {
      if (redis) await redis.set(cacheKey, JSON.stringify(result), 'EX', 90000);
    } catch {
      // Redis unreachable — skip write, next request will call Claude again.
    }
